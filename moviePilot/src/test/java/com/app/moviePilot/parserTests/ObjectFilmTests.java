...
class ObjectFilmTests {
	...
	static void setup() {
		...
		filmJson = filmParser.getJson("https://api.themoviedb.org/3/movie/550?api_key=6cacd119a397de0ec8845d760efdb7ab");
	...
	void testAttributes() {
		...
		assertEquals("A ticking-time-bomb insomniac and a slippery soap salesman channel primal male aggression into a shocking new form of therapy. Their concept catches on, with underground \"fight clubs\" forming in every town, until an eccentric gets in the way and ignites an out-of-control spiral toward oblivion.", film.getOverview());
	...
	void testAttributesNotNull() {
		...
		assertNotNull("A ticking-time-bomb insomniac and a slippery soap salesman channel primal male aggression into a shocking new form of therapy. Their concept catches on, with underground \"fight clubs\" forming in every town, until an eccentric gets in the way and ignites an out-of-control spiral toward oblivion.", film.getOverview());
		assertNotNull(film.getAdult());
		assertNotNull(film.getPosterPath());
		assertNotNull(film.getReleaseDate().toString());
	...
	void testObjectToJson() {
		...
		assertEquals(jsonToSend.getAsJsonObject().get("genres").getAsJsonArray().size(), filmJson.getAsJsonObject().get("genres").getAsJsonArray().size(), " ");
		...
		assertEquals(jsonToSend.getAsJsonObject().get("voteAverageApi"), filmJson.getAsJsonObject().get("vote_average"));
	...
	void testListObjectByPage() {
		...
		assertEquals(502356, listFilm.get(0).getIdApi());
	}
